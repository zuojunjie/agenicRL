#!/usr/bin/env bash
# Phase 7.6 = Phase 7 inherit (= Phase 6 GSPO 配置) + 52-step subset + eval=false
#
# 与 Phase 7.5（4090 严格对齐）相比，回到 Phase 6 验证过的高速学习曲线：
#   - policy_loss_type=gspo                     (vs 7.5 regular)
#   - loss_reduction=default(token_mean)        (vs 7.5 sequence_mean)
#   - ppo_mini_batch=64 / micro=8               (vs 7.5 256/32)
#   - flash_attn=false / use_sample_packing=false (vs 7.5 true/true)
#   - gpu_memory_utilization=0.18               (vs 7.5 0.30)
#
# 与 Phase 7（继承）的差异：
#   - data: train.parquet 79K → train_52steps.parquet 26K
#   - epochs: 15 → 1
#   - eval_before_train: true → false
#
# Phase 6 reward 曲线（参照）：step 1=0.14, step 2=0.19, step 3=0.30, step 4=0.30, step 5=0.33
# wall clock: ~17 min/step × 52 = ~14.7h（dual GPU 受 ppo_mini=64+micro=8 限制不快）

set -ex
ulimit -n 65535

DATA_DIR="/root/autodl-tmp/data/searchR1"
RUN_NAME="phase7_6-skyrl-pro6k-dual-gspo-52step"
...
# Dual-GPU NCCL
# 注意：不要 set PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
# (与 vllm v1 CuMemAllocator 冲突，启动时会 assert 退出)
...
uv run --isolated --frozen --extra fsdp -m skyrl.train.entrypoints.main_base \
    ...
    \
    `# === Algorithm: GSPO native (Phase 6 验证可学习路径) ===` \
    ...
    \
    `# === Batch: Phase 6 inherit (mini=64, micro=8) ===` \
    trainer.epochs=1 \
