#!/usr/bin/env bash
# Phase 7.5 = Phase 6 = E (PRO 6000 #1) + 12 changes (full 4090 strict alignment):
#
# Algorithm (4 changes):
#   1. policy_loss_type:    gspo → regular         (verl vanilla equivalent)
#   2. loss_reduction:      [default] → sequence_mean (verl sequence equivalent)
#
# Batch (3 changes):
#   3. ppo_mini_batch_size:           64 → 256       (4090 baseline)
#   4. micro_forward_batch_size_per_gpu: 8 → 32      (4090 baseline)
#   5. micro_train_batch_size_per_gpu:   8 → 32      (4090 baseline)
#
# Optim (2 changes):
#   6. use_sample_packing:  false → true            (verl use_remove_padding)
#   7. flash_attn:          false → true            (sample_packing requires it)
#
# GPU+Mem (2 changes):
#   8. policy/ref_num_gpus + TP:        1 → 2       (dual GPU)
#   9. gpu_memory_utilization:          0.18 → 0.30
#
# Data (2 changes):
#   10. epochs:             15 → 1                  (52-step subset workaround)
#   11. data.train_data:    train_52steps.parquet  (26624 rows = 52 steps × 512)
#       注：SkyRL 无 max_steps key，total_steps=dataloader_len×epochs 强绑定，
#           所以只能靠子集 parquet 做 52 步。配置 loss/algo 已与 4090 严格对齐。
#
# Misc (1 change):
#   12. ckpt_interval:      50 → 10                 (4090 save_freq=10)
#
# ENV vars added:
#   - PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True  (verl 4090 sets this)
#   - +generator.engine_init_kwargs.attention_backend=xformers (verl XFORMERS)

set -ex
ulimit -n 65535

DATA_DIR="/root/autodl-tmp/data/searchR1"
RUN_NAME="phase7_5-skyrl-pro6k-dual-aligned-maxturn2"
...
# NCCL only (注意：PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True 与 vllm
# CuMemAllocator 不兼容 — vllm v1 启动时会 assert，所以**不能**像 4090 那样设)
...
uv run --isolated --frozen --extra fsdp -m skyrl.train.entrypoints.main_base \
    ...
    \
    `# === Algorithm: 4090 verl vanilla equivalent ===` \
    ...
    \
    `# === Inference: dual GPU TP=2 + xformers attention ===` \
    ...
    \
    `# === Logging + ckpt: 4090 equivalent (save_freq=10) ===` \
