#!/usr/bin/env bash
# Phase 9 = 7.8d 最优配置 + Qwen2.5-7B + 2× PRO 6000 + CPU offload
#
# 改动 (vs 7.8d):
#   - 模型: Qwen2.5-3B-Instruct → Qwen2.5-7B-Instruct
#   - cpu_offload: false → true (FSDP optim 移到 CPU 释放 ~30 GB/GPU)
#   - micro: 32 → 16 (减半留显存余地)
#   - gpu_memory_utilization: 0.50 → 0.40 (7B vllm 占更多)
#
# 继承全部最优算法 (7.8d):
...
#
# 内存预算 (per GPU 96 GB):
#   FSDP 7B sharded + cpu_offload Adam:    ~50 GB
#   FSDP all-gather + activations:         ~25 GB
#   retrieval_server:                      ~9 GB
#   缓冲:                                  ~12 GB
#
# 期望:
#   单 step:  ~15-18 min
#   52 步:    ~13-16 h
#   val:      0.46-0.52 (Search-R1 paper 7B 报 ~0.50-0.55)

set -ex
ulimit -n 65535

DATA_DIR="/root/autodl-tmp/data/searchR1"
RUN_NAME="phase9-skyrl-pro6k-dual-7B-cpuoffload"
