#!/usr/bin/env bash
# Phase 5a — ARPO: trajectory credit + adaptive entropy
# 与 0e 严格 single-variable 对比（不带 GSPO/GiGPO/DAPO/ToRL）
#
# 前置:
#   1. dp_actor.py 已 apply phase5a patch
...
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-phase5a-arpo-trajcredit-adaptent-52}
export WAND_PROJECT='agentic-rl-search'

# === ARPO 5a 关键开关 ===
export ARPO_TRAJ_CREDIT=1
export ARPO_ADAPT_ENTROPY=1
export ARPO_ENTROPY_TARGET=1.0

# 不带 GSPO / GiGPO / DAPO（确保 single-variable）
unset GIGPO_ALPHA
...
echo "[phase5a] ARPO traj_credit=$ARPO_TRAJ_CREDIT adapt_entropy=$ARPO_ADAPT_ENTROPY target=$ARPO_ENTROPY_TARGET"
