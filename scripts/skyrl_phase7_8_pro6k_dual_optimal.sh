#!/usr/bin/env bash
# Phase 7.8 = Phase 7.5 base + 单独修复 mini_batch (二分搜索锁定的元凶)
#
# 二分搜索结论：policy_mini_batch_size=256 是 Phase 7.5 学习慢的唯一关键变量
#   - 7.7-A (gspo+token_mean+mini=256):  step 4 reward = 0.168  ❌ 不涨
#   - 7.7-B (regular+seq_mean+mini=64):  step 4 reward = 0.283  ✅ 与 Phase 6 同步
#
# Phase 7.8 配置 = "最优混合"：
#   - 4090 对齐：regular + sequence_mean + flash_attn + sample_packing + gpu_mem=0.30
#   - Phase 6 修复：ppo_mini_batch=64
#   - 微调：micro=32（4090 速度等价，数学与 micro=8 等价）
#
# 预期：reward 轨迹 ≈ Phase 6（step 5 ~0.33, step 50 ~0.43）
#       wall time ≈ 6.2 min/step（mini=64 在 dual 上每步多 8 grad updates）
#       52 步 ≈ 5.4 h
#
# 与 Phase 7.5 v3 (5.27 min/step, mini=256) 比慢 ~17%, 但 reward 学得快得多

set -ex
ulimit -n 65535

DATA_DIR="/root/autodl-tmp/data/searchR1"
RUN_NAME="phase7_8c-skyrl-pro6k-dual-optimal-mini64-cudagraph"
