# Search-R1 Agent Loop · 代码层心智模型

读 `external/Search-R1/infer.py` (135 行) + `search_r1/search/retrieval_server.py` (393 行) 的精读笔记。
...
**要在 Mac 上跑，必须 patch 成 device-aware**。但我们不在本地跑真服务器——直接 mock 即可。
